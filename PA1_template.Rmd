...
```
There were `r missingValues` to be exact. And some entire days have no values for steps taken. So imputing values required some thought. I decided that I would take the mean number of steps for that time interval and day and use that to impute the missing values into the activity data.
...
This showed that there was more focused activity during the weekday around the 8:00 am hour (probably a dailly workout) but the weekend spread the activity out over the day (probably not sitting at a desk like durin the week).
